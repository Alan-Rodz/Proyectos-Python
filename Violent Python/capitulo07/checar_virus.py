...
#Este programa sube el archivo que recibe a NoVirusThanks y checa si puede ser detectado por antivirus populares

def imprimir_resultados(url):
    status = 200
    host = urlparse(url)[1]
    path = urlparse(url)[2]
...
def subir_archivo(filename):
    print("[+] Subiendo archivo a NoVirusThanks...")
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(_filename):
        ...
        loc = subir_archivo(_filename)
        imprimir_resultados(loc)
